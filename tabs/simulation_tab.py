...
def render() -> None:
    ...
    try:
        ...
        st.error(f"Could not load layout JSON: {e}")
        st.info("If this came from an API/file response, try broadening the output format and ensure a valid JSON object is saved.")
        return
    ...
    if st.button("Load preset", key="sim_load_preset_btn"):
        try:
            ...
            st.info("Try re-saving the preset. If generated by an API, ensure it returns a JSON object (not empty text).")
        else:
